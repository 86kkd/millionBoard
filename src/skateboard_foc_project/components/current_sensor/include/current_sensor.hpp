...
public:
  ...
  */
  ESP32InlineCurrentSense(float shunt_resistor, float gain, int pinA, int pinB,
                          int pinC = NOT_SET);
  /**
    InlineCurrentSense class constructor
    ...
    @param phC C phase adc pin (optional)
  */
  ESP32InlineCurrentSense(float mVpA, int pinA, int pinB, int pinC = NOT_SET);
  ...
  // LowPassFilter lpf_a{DEF_LPF_PER_PHASE_CURRENT_SENSE_Tf}; //!<  current A
  // low pass filter LowPassFilter lpf_b{DEF_LPF_PER_PHASE_CURRENT_SENSE_Tf};
  // //!<  current B low pass filter LowPassFilter
  // lpf_c{DEF_LPF_PER_PHASE_CURRENT_SENSE_Tf}; //!<  current C low pass filter
