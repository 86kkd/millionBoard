...
static const char *kTag = "pressure_sensor";
...
void app_main(void) {
  ...
  PressureSensor pressure_sensor_1(hx711_1);
  PressureSensor pressure_sensor_2(hx711_2);

  // 尝试从NVS加载校准参数
  bool loaded_1 =
      pressure_sensor_1.LoadCalibration(kNvsNamespace, kScalePrefix1);
  bool loaded_2 =
      pressure_sensor_2.LoadCalibration(kNvsNamespace, kScalePrefix2);
  // 根据宏选择是否已加载要校准的传感器参数
...
// 仅对指定传感器进行零点校准
      ...
      if (pressure_sensor_1.Tare(10)) {
#else
      if (pressure_sensor_2.Tare(10)) {
#endif
        ESP_LOGI(kTag,
...
// 仅对指定传感器进行比例系数校准
#if SENSOR_TO_CALIBRATE == 1
      if (pressure_sensor_1.Calibrate(kKnownWeight, 10)) {
        pressure_sensor_1.SaveCalibration(kNvsNamespace, kScalePrefix1);
#else
      if (pressure_sensor_2.Calibrate(kKnownWeight, 10)) {
        pressure_sensor_2.SaveCalibration(kNvsNamespace, kScalePrefix2);
#endif
    ...
    case kMeasurement:
      // 读取重量并显示
      weight_1 = pressure_sensor_1.GetWeight(5);
      weight_2 = pressure_sensor_2.GetWeight(5);
      ...
      ESP_LOGI(kTag,
               ...
               pressure_sensor_1.GetOffset(), pressure_sensor_1.GetScale(),
               pressure_sensor_2.GetOffset(), pressure_sensor_2.GetScale());
