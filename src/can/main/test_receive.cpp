...
extern "C" void app_main(void) {
  ...
  ESP_LOGI(TAG,
           "TWAI driver installed and started in listen-only mode at 100kbps");
  ...
  while (1) {
    ...
    if (recv_res == ESP_OK) {
      ESP_LOGI(TAG,
               "Message received - ID: 0x%03X, DLC: %d, Data: 0x%02X 0x%02X "
               "0x%02X 0x%02X 0x%02X 0x%02X 0x%02X 0x%02X",
               rx_msg.identifier, rx_msg.data_length_code, rx_msg.data[0],
               rx_msg.data[1], rx_msg.data[2], rx_msg.data[3], rx_msg.data[4],
