...
try:
    ...
    print("PySerial not properly installed. Please install it with:")
...
class SH367601X:
    ...
    def __init__(self, port, baudrate=9600, timeout=1, debug=False, max_retries=3):
        """初始化串口连接
        ...
        try:
            ...
            if not any(p.device == port for p in ports):
                print(f"设备 {port} 不存在。可用的串口设备:")
                for p in ports:
                    print(f"  - {p.device}: {p.description}")
            # 检查权限问题
            elif "Permission denied" in str(e):
                print(f"无权访问 {port}，请尝试以下命令:")
    ...
    def send_command(self, cmd, data=None, retries=0):
        """发送命令并接收响应
        ...
        if retries > self.max_retries:
            if self.debug:
                print(f"命令 0x{cmd:02X} 达到最大重试次数 {self.max_retries}")
            return None
        ...
        if self.debug:
            print(f"发送数据: {' '.join([f'{b:02X}' for b in frame])}")
        ...
        try:
            # 发送命令
            bytes_written = self.ser.write(bytes(frame))

            if self.debug:
                print(f"写入 {bytes_written} 字节")

            # 等待响应
            time.sleep(0.1)  # 增加延迟，给设备更多响应时间
            ...
            if self.debug:
                if response:
                    print(f"接收数据: {' '.join([f'{b:02X}' for b in response])}")
                else:
                    print("未接收到数据")

            if len(response) < 3:  # 至少应该有地址、命令和CRC
                print(f"接收响应失败，接收到 {len(response)} 字节")
                if self.debug and retries < self.max_retries:
                    print(f"尝试重试 ({retries+1}/{self.max_retries})...")
                return (
                    self.send_command(cmd, data, retries + 1)
                    if retries < self.max_retries
                    else None
                )
            ...
            if received_crc != calculated_crc:
                print(
                    f"CRC校验失败: 接收={received_crc:02X}, 计算={calculated_crc:02X}"
                )
                if self.debug and retries < self.max_retries:
                    print(f"尝试重试 ({retries+1}/{self.max_retries})...")
                return (
    ...
    def read_cell_voltages(self):
        ...
        for i in range(16):
            ...
            if response and len(response) >= 2:
                ...
                voltage = (
                    int.from_bytes(response, byteorder="big", signed=True) * 5 / 32
                )
    ...
    def test_communication(self):
        ...
        if response is not None:
            ...
            print("通信测试失败，无法与芯片通信")
            print("请检查:")
            print("1. 电池保护板是否正确连接")
            print("2. USB转串口适配器是否正常工作")
            print("3. 波特率是否正确(默认9600)")
            return False
...
def main():
    ...
    parser.add_argument("port", nargs="?", help="串口设备名称")
    ...
    parser.add_argument(
        "--baudrate", type=int, default=9600, help="设置波特率(默认9600)"
    )
    parser.add_argument("--test", action="store_true", help="测试与芯片的通信")
    ...
    if not args.port:
        parser.error("当不使用 --list-ports 时，port 参数是必需的")

    try:
        chip = SH367601X(args.port, baudrate=args.baudrate, debug=args.debug)

        if args.test:
            success = chip.test_communication()
            if not success:
                return
